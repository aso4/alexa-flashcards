...
class AlexaResponse
  ...
  def append_attribute(key, value)
    @session_attributes[key] << value unless @session_attributes[key].nil?
  end
  ...
  def link_card
    with_card.tap { |hs| hs['response']['card'] = { 'type' => 'LinkAccount' } }
  end

  def without_card
    with_card.tap { |hs| hs['response'].delete('card') }
  end
  end
